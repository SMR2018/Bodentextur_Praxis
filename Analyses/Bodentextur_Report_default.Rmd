...
```{r,  warning=FALSE, fig.height=6, fig.width=8, fig.cap=cap2, fig.align='center'}
#, out.height="400px"
...
Klasse_uebersicht <- df_bodenklasse %>%
        group_by(Bodenklasse) %>%
        summarise(Flaechen_anteil = round(n()/nrow(.)*100,1)) %>%
        left_join(alle_klassen, by=(c("Bodenklasse" = "Nr"))) %>%
        arrange(desc(Flaechen_anteil)) %>%
        select(Bodenklasse, abbr, name, Flaechen_anteil)
...
kable(Klasse_uebersicht, "latex",
      ...
      caption = "Flächenanteil der beobachteten Bodentextur-Klassen."
        ...
        as_image(file = "Tabelle1.png",
                 latex_header_includes=c("\\usepackage[justification=raggedright,singlelinecheck=false]{caption}", " \\captionsetup[table]{name=Tabelle}" )) #um im mehrsplatigen PDF zu drucken
```
:::
...
raster_bodenklasse <- rasterFromXYZ(df_bodenklasse, crs = p1)
...
```{r, warning=FALSE, fig.height=2.5, out.width='60%', fig.cap=cap, fig.align='center'}
...
```{r, fig.width=6, fig.height=6, out.width='60%', fig.align='center', fig.cap=cap2}
